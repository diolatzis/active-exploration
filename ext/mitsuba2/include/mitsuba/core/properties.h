...
public:  // Type-specific getters and setters ----------------------------------
    ...
    void set_bool(const std::string &name, const bool &value, bool warn_duplicates = false);
    ...
    void set_int(const std::string &name, const int &value, bool warn_duplicates = false) {
    ...
    void set_long(const std::string &name, const int64_t &value, bool warn_duplicates = false);
    ...
    void set_float(const std::string &name, const Float &value, bool warn_duplicates = false);
    ...
    void set_string(const std::string &name, const std::string &value, bool warn_duplicates = false);
    ...
    void set_named_reference(const std::string &name, const NamedReference &value, bool warn_duplicates = false);
    ...
    void set_array3f(const std::string &name, const Array3f &value, bool warn_duplicates = false);
    /// Retrieve a 3D array
    ...
    /// Store a 3D array in the Properties instance
    void set_array4f(const std::string &name, const Array4f &value, bool warn_duplicates = false);
    /// Retrieve a 3D array
    Array4f array4f(const std::string &name) const;
    /// Retrieve a 3D array (use default value if no entry exists)
    Array4f array4f(const std::string &name, const Array4f &def_val) const;
    ...
    /// Retrieve a 3D vector
    Vector4f vector4f(const std::string &name) const { return array4f(name); }
    /// Retrieve a 3D vector (use default value if no entry exists)
    Vector4f vector4f(const std::string &name, const Vector4f &def_val) const { return array4f(name, def_val); }

    /// Store a 4x4 homogeneous coordinate transformation in the Properties instance
    void set_transform(const std::string &name, const Transform4f &value, bool warn_duplicates = false);
    ...
    void set_object(const std::string &name, const ref<Object> &value, bool warn_duplicates = false);
    ...
    void set_pointer(const std::string &name, const void * const &value, bool warn_duplicates = false);
    ...
    void set_color(const std::string &name, const Color3f &value, bool warn_duplicates = false);
    /// Retrieve a color
    ...
    template <typename Texture>
    std::vector<ref<Texture>> textures(const std::string &name) const {
        if (!has_property(name))
            Throw("Property \"%s\" has not been specified!", name);

        auto p_type = type(name);
        if (p_type == Properties::Type::Object) {
            std::vector<ref<Object>> objects = find_objects(name);
            if (!objects[0]->class_()->derives_from(MTS_CLASS(Texture)))
                Throw("The property \"%s\" has the wrong type (expected "
                    " <spectrum> or <texture>).", name);
            ...
            {   
                textures.push_back((Texture*)object.get());
                ++i;
            }
            return textures;
...
private:
    ...
    std::vector<ref<Object>> find_objects(const std::string &name) const;
