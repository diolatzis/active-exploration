...
public:
    ...
    SmoothDiffuse(const Properties &props) : Base(props) {
        ...
        m_reflectances = props.textures<Texture>("reflectance");
        m_flags = BSDFFlags::DiffuseReflection | BSDFFlags::FrontSide;
        m_modifier = Color3f(1.0f, 1.0f, 1.0f);
    ...
    std::pair<BSDFSample3f, Spectrum> sample(const BSDFContext &ctx,
        ...
        UnpolarizedSpectrum value = m_modifier * m_reflectance->eval(si, active);
    ...
    Spectrum eval(const BSDFContext &ctx, const SurfaceInteraction3f &si,
        ...
        UnpolarizedSpectrum value =
            m_modifier * m_reflectance->eval(si, active) * math::InvPi<Float> * cos_theta_o;
    ...
    int num_alternatives() const override {
        return m_reflectances.size();
    }

    void set_alternative(int index) override {
        if(index < m_reflectances.size()) m_reflectance = m_reflectances[index];
    }

    Color3f get_reflectance(BSDFContext &ctx, SurfaceInteraction3f& si) const { return m_modifier * m_reflectance->eval(si, true); }

    Float get_alpha(SurfaceInteraction3f& si) const { return 1.0f; }


    void set_modifier(Color3f modifier) override {
        m_modifier = modifier;
    }
...
private:
    ...
    std::vector<ref<Texture>> m_reflectances;
};
