...
public:
    ...
    Color3f get_reflectance(BSDFContext &ctx, SurfaceInteraction3f &si) const { 

        bool has_specular = ctx.is_enabled(BSDFFlags::GlossyReflection, 0),
            has_diffuse  = ctx.is_enabled(BSDFFlags::DiffuseReflection, 1);

        Color3f value = Color3f(1.0f, 1.0f, 1.0f);

        if (m_specular_reflectance) value *= m_specular_reflectance->eval(si, true);
        if (m_diffuse_reflectance) value *= m_diffuse_reflectance->eval(si, true);
