...
public:
    ...
    PointLight(const Properties &props) : Base(props) {
        if (props.has_property("position")) {
            ...
            m_vertex_positions_buf = FloatStorage::copy(props.point3f("position").data(), 3);
    ...
    void parameters_changed(const std::vector<std::string>& keys) override {
        if (keys.empty() || string::contains(keys, "vertex_positions_buf")) {
            ...
            ScalarVector3f position = ScalarVector3f(m_vertex_positions_buf[0], m_vertex_positions_buf[1], m_vertex_positions_buf[2]);

            m_world_transform = new AnimatedTransform(
                ScalarTransform4f::translate(position));

            m_bbox = ScalarBoundingBox3f(position);
