...
class AOVBounceIntegrator final : public SamplingIntegrator<Float, Spectrum> {
public:
    ...
    AOVBounceIntegrator(const Properties &props) : Base(props) {
        m_max_depth = props.int_("max_depth", 0);
    ...
    std::pair<Spectrum, Mask> sample(const Scene *scene,
        ...
        // Sample an extra bounce
        BSDFContext bsdfContext; 

        if (m_max_depth == 1)
        {
            auto [bs, bsdf_val] = si.bsdf()->sample(bsdfContext, si, sampler->next_1d(active), sampler->next_2d(active), active);
            si[has_flag(si.bsdf()->flags(), BSDFFlags::DeltaReflection) && !has_flag(si.bsdf()->flags(), BSDFFlags::DeltaTransmission)] = scene->ray_intersect(si.spawn_ray(si.to_world(bs.wo)));
        }
        else
        {
            int depth = 0;

            while (any_nested(has_flag(si.bsdf()->flags(), BSDFFlags::DeltaReflection)) && depth < m_max_depth)
            {
                // Sample BSDF * cos(theta)
                auto [bs, bsdf_val] = si.bsdf()->sample(bsdfContext, si, sampler->next_1d(active), sampler->next_2d(active), active);
                si[has_flag(si.bsdf()->flags(), BSDFFlags::DeltaReflection)] = scene->ray_intersect(si.spawn_ray(si.to_world(bs.wo)));
                ++depth;
            }
        }
        ...
        for (size_t i = 0; i < m_aov_types.size(); ++i) {
            switch (m_aov_types[i]) {
                ...
                case Type::Albedo:
                {
                    ...
                    auto albedo = si_copy.bsdf()->get_reflectance(bsdfContext, si_copy);
                ...
                case Type::Emission:
                {
                    ...
                    auto emitter = full_si.emitter(scene);
                    
                    if (any_or<true>(neq(emitter, nullptr))) emission = emitter->eval(full_si, active);
                ...
                case Type::Wi:
                {
                    ...
                    si.initialize_sh_frame(arbitrary_s);
                    auto local_wi = si.sh_frame.to_local(world_wi);
                    *aovs++ = select(has_flag(si.bsdf()->flags(), BSDFFlags::GlossyDelta), ((local_wi.x()+1.0f)/2.0f), 0.0f);
                    *aovs++ = select(has_flag(si.bsdf()->flags(), BSDFFlags::GlossyDelta), ((local_wi.y()+1.0f)/2.0f), 0.0f);
                    *aovs++ = select(has_flag(si.bsdf()->flags(), BSDFFlags::GlossyDelta), abs(local_wi.z()), 0.0f);
                    break;
                }
...
MTS_IMPLEMENT_CLASS_VARIANT(AOVBounceIntegrator, SamplingIntegrator)
MTS_EXPORT_PLUGIN(AOVBounceIntegrator, "AOV bounce integrator");
