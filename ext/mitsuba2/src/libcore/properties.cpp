...
struct Properties::PropertiesPrivate {
    ...
    std::multimap<std::string, Entry, SortKey> duplicate_entries;
...
#define DEFINE_PROPERTY_ACCESSOR(Type, TagName, SetterName, GetterName) \
    void Properties::SetterName(const std::string &name, Type const &value, bool error_duplicates) { \
        ...
        e.data = (Type) value; \
        e.queried = false; \
        d->duplicate_entries.insert(std::pair<std::string, Entry>(name, e)); \
...
namespace {
    struct PropertyTypeVisitor {
        ...
        Type operator()(const Array4f &) { return Type::Array4f; }
...
std::vector<std::pair<std::string, NamedReference>> Properties::named_references() const {
    ...
    result.reserve(d->duplicate_entries.size());
    for (auto &e : d->duplicate_entries) {
...
/// Array4f setter
void Properties::set_array4f(const std::string &name, const Array4f &value, bool error_duplicates) {
    if (has_property(name) && error_duplicates)
        Log(Error, "Property \"%s\" was specified multiple times!", name);
    d->entries[name].data = (Array4f) value;
    d->entries[name].queried = false;
}

/// Array4f getter (without default)
Array4f Properties::array4f(const std::string &name) const {
    const auto it = d->entries.find(name);
    if (it == d->entries.end())
        Throw("Property \"%s\" has not been specified!", name);
    if (!it->second.data.is<Array4f>())
        Throw("The property \"%s\" has the wrong type (expected <vector> or <point>).", name);
    it->second.queried = true;
    return it->second.data.operator Array4f&();
}

/// Array4f getter (with default)
Array4f Properties::array4f(const std::string &name, const Array4f &def_val) const {
    const auto it = d->entries.find(name);
    if (it == d->entries.end())
        return def_val;
    if (!it->second.data.is<Array4f>())
...
std::vector<ref<Object>> Properties::find_objects(const std::string& name) const
{
    std::vector<ref<Object>> objects;

    const auto res_range = d->duplicate_entries.equal_range(name);

    for (auto it = res_range.first; it != res_range.second; ++it)
    {
        if (it->second.data.is<ref<Object>>()) objects.push_back(it->second.data);
    }
