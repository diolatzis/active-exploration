...
MTS_VARIANT BSDF<Float, Spectrum>::BSDF(const Properties &props)
    : m_flags(+BSDFFlags::None), m_id(props.id()), m_num_parameters(props.int_("num_parameters", 3)), m_min_bounds(props.array4f("min_bounds", 0.0f)), m_range_bounds(props.array4f("range_bounds", 1.0f)) { }
