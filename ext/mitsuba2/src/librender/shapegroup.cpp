...
MTS_VARIANT ShapeGroup<Float, Spectrum>::ShapeGroup(const Properties &props) {
    ...
    for (auto &kv : props.objects()) {
        ...
        if (c_class->derives_from(MTS_CLASS(Base))) {
...
#endif
    SurfaceInteraction3f si = pi.shape->compute_surface_interaction(m_to_object.transform_affine(ray), pi, flags, active);
...
MTS_VARIANT void ShapeGroup<Float, Spectrum>::optix_prepare_ias(
    ...
    prepare_ias(context, m_shapes, m_sbt_offset, m_accel, instance_id, transf * m_to_world, instances);
}
