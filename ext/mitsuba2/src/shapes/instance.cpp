...
public:
    MTS_IMPORT_BASE(Shape, m_id, m_to_world, m_original_to_world, m_num_parameters, m_to_object, m_min_bounds, m_range_bounds, m_emitter, set_children, m_rotation_axis, m_range_angle, m_min_angle)
    ...
    Instance(const Properties &props) {
        ...
        m_original_to_world = m_to_world;
        ...
        m_rotation_axis = props.vector3f("rotation_axis", 1.0f);
        m_range_angle = props.float_("range_angle", 360.0f);
        m_min_angle = props.float_("min_angle", 0.0f);
        ...
        for (auto &kv : props.objects()) {
            ...
            if (shape && shape->is_shapegroup()) {
                ...
                if (m_shapegroup->has_emitter()) m_emitter = m_shapegroup->get_emitter()->emitter();
            } else {
    ...
    PositionSample3f sample_position(Float time, const Point2f& sample, Mask active = true) const
    {
        ...
        ps.p = m_to_world.transform_affine(ps.p);
        ps.n = m_to_world.transform_affine(ps.n);
    ...
    Float pdf_position(const PositionSample3f& ps, Mask active = true) const {
        ...
        ps_new.p = m_to_object.transform_affine(ps_new.p);
        ps_new.n = m_to_object.transform_affine(ps_new.n);

        return m_shapegroup->get_emitter()->pdf_position(ps_new, active);
    }

    void traverse(TraversalCallback* callback) override { m_shapegroup->traverse(callback); }
    void parameters_changed(const std::vector<std::string> &/*keys*/ = {}) override {}

    int num_alternatives() const override { return m_shapegroup->num_alternatives(); }

    void set_alternative(int index) override { m_shapegroup->set_alternative(index); }
